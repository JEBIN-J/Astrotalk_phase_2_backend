...
class TarotEngine:
    ...
    @staticmethod
    def draw_cards(num_cards: int, seed=None):
    ...
    @staticmethod
    def interpret_card(card: dict, context: str = "general") -> dict:
        """Returns ALL card fields so the Flutter frontend can display real data."""
        ...
        return {
            ...
            "arcana_type": card.get("arcana_type"),
            "suit": card.get("suit"),
            "rank": card.get("rank"),
            ...
            "relationship_meaning": card.get("relationship_meaning"),
            "marriage_meaning": card.get("marriage_meaning"),
            "career_meaning": card.get("career_meaning"),
            "education_meaning": card.get("education_meaning"),
            "finance_meaning": card.get("finance_meaning"),
            "health_general_meaning": card.get("health_general_meaning"),
            "spiritual_meaning": card.get("spiritual_meaning"),
            "travel_meaning": card.get("travel_meaning"),
            "past_meaning": card.get("past_meaning"),
            "present_meaning": card.get("present_meaning"),
            "future_meaning": card.get("future_meaning"),
            ...
            "astrology_correspondence": card.get("astrology_correspondence"),
            "element": card.get("element"),
        }

    @staticmethod
    def create_spread(spread_type: str, seed=None) -> dict:
        ...
        spread_configs = {
            ...
            "celtic_cross": {"count": 10, "positions": ["Present", "Challenge", "Foundation", "Past", "Goal", "Near Future", "Self", "Environment", "Hopes/Fears", "Outcome"]},
            "year_ahead": {"count": 12, "positions": ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]},
            "astrological_houses": {"count": 12, "positions": ["1st House (Self)", "2nd House (Wealth)", "3rd House (Communication)", "4th House (Home)", "5th House (Creativity)", "6th House (Health/Work)", "7th House (Partnerships)", "8th House (Transformation)", "9th House (Philosophy/Travel)", "10th House (Career)", "11th House (Network)", "12th House (Subconscious)"]}
        }
        ...
        for i, pos_name in enumerate(config["positions"]):
            ...
            positions_data.append({"position_name": pos_name, "card": card_info})
        return {"spread_type": spread_type, "seed": used_seed, "positions": positions_data}
